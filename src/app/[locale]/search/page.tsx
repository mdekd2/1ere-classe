        ...
        <div className="card mb-8">
          <div className="flex items-center justify-between mb-4">
            ...
            >
              ...
              <span>{showFilters ? tCommon('close') : tCommon('open')} {t('filters')}</span>
            </button>
          </div>
          ...
          <div className={`grid grid-cols-1 md:grid-cols-4 gap-4 ${showFilters ? 'block' : 'hidden'}`}>
            <div className="relative">
              ...
              >
                <option value="">{t('from')} ({tCommon('open')})</option>
                {cities.map(city => (
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-transparent"
              >
                <option value="">{t('to')} ({tCommon('open')})</option>
                {cities.map(city => (
                  <option key={city} value={city}>{city}</option>
                ))}
              </select>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center space-x-6 text-sm text-gray-600 dark:text-gray-300">
                        <div className="flex items-center space-x-1">
                          ...
                          <span>{t('departure')}: {formatTime(trip.departureTime)}</span>
                        </div>
                        <div className="flex items-center space-x-1">
                          <Clock className="w-4 h-4" />
                          <span>{t('arrival')}: {formatTime(trip.arrivalTime)}</span>
                        </div>
                      </div>

                      <div className="flex items-center space-x-6 text-sm text-gray-600 dark:text-gray-300">
                        <span>{t('duration')}: {formatDuration(trip.route?.estimatedDuration || 0)}</span>
                        <span>{t('distance')}: {trip.route?.distance} km</span>
                      ...
                      <div className="flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-300">
                        <div className="flex items-center space-x-1">
                          ...
                          <span>{trip.availableSeats} {t('seatsAvailable')}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-1">
                      ...
                      {trip.bus?.amenities.length && trip.bus.amenities.length > 3 && (
                        <div className="text-sm text-gray-500">
                          +{trip.bus.amenities.length - 3} {t('moreAmenities')}
                        </div>
                      )}
                    </div>
                  </div>
